(function() {
  ...
  _.each = function(collection, iterator) {
    //call Array.isArray to determine if the passed argument is an array or an object
    if (Array.isArray(collection)){
      //run for loop for arrays
      for (var i = 0; i < collection.length; i++) {
        iterator(collection[i], i, collection);
      }
    } else {
      //run for in loop for objects
  ...
  _.reduce = function(collection, iterator, accumulator) {
    ...
    //if collection is an object (not an array) then create an array of keys and loop through
    //as defined by the rules of .reduce()
    if (Array.isArray(collection)){  

      //set accumulator to the first value in the array if accumulator is undefined
      //also set the first iteration of the array loop to the second item in the array
      //if accumulator is defined start the loop at the first element of the array
      ...
      //if accumulator is defined start the loop at the first element of the array
      if (typeof accumulator === 'undefined') {
        accumulator = collection[keyArray[0]];
        var i = 1;
      } else {
        var i = 0;
      }

      //loop through object's properties starting at the correct point
  ...
  _.memoize = function(func) {
    var alreadyCalled = false;
    var result;
    var args = [];  //variable to store the arguments of the previous call

    return function() {
      //compare arguments of previous call to arguments of current call
      ...
      if (!alreadyCalled) {
        args = arguments;  //set args variable for future calls
        result = func.apply(this, arguments);
